...
import { Heart, BookOpen, BarChart3, HelpCircle, ShieldAlert, Sparkles, Smile, RefreshCw } from 'lucide-react';
...
export default function App() {
  ...
  // Load from LocalStorage and apply pre-seeds if empty
  useEffect(() => {
    const savedLogs = localStorage.getItem('pet-mood-logs');
    const savedPet = localStorage.getItem('pet-mood-state');
    ...
    if (savedLogs) {
      ...
      // Seed data so charts are prepackaged with interactive values
      ...
      localStorage.setItem('pet-mood-logs', JSON.stringify(initialLogs));
    }
    ...
    if (savedPet) {
      ...
      localStorage.setItem('pet-mood-state', JSON.stringify(defaultState));
    }
  }, []);

  // Save changes to localStorage on mutations
  const updateAndSaveLogs = (newLogs: MoodLog[]) => {
    setLogs(newLogs);
    localStorage.setItem('pet-mood-logs', JSON.stringify(newLogs));
    if (newLogs.length > 0) {
  ...
  const updateAndSavePet = (newPet: PetState) => {
    setPetState(newPet);
    localStorage.setItem('pet-mood-state', JSON.stringify(newPet));
  };
  ...
  const handleResetApp = () => {
    if (window.confirm('確定要重新孵化新的寵物並清除日記紀錄嗎？此動作將重置所有本機資料。')) {
      localStorage.removeItem('pet-mood-logs');
      localStorage.removeItem('pet-mood-state');
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-slate-400 hover:text-rose-600 p-2.5 hover:bg-rose-50 border-2 border-slate-100 hover:border-slate-900 rounded-xl shadow-sm hover:shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] transition-all"
              title="重置系統"
            >
